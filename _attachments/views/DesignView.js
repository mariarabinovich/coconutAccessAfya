...
DesignView = (function(_super) {
  ...
  DesignView.prototype.template = Handlebars.compile("<div id='design-view'> <h3>Design</h3> <small > <b class='instructions'>Instructions</b>: <p class='instructions'>Use the drop down below to select the type of questions that you will be asking. Click <button>Preview</button> to see what the questions will look like.</p> </small> <hr/> <div class='sortable' id='questions'> <label class='questionText' for='rootQuestionName'>Name</label> <input class='designFormTextInput' id='rootQuestionName' name='rootQuestionName' type='text'/> </div> <div class='designControls'> <label for='element_selector'>Add questions</label> <select id='element_selector'> {{#each types}} <option>{{this}}</option> {{/each}} </select> <button class='designActionsButton addButton'>Add</button><br/> <button class='designActionsButton' type='button'>Save</button> <button class='designActionsButton'>Preview</button> <button class='designActionsButton'>Advanced Mode</button> </div> <form id='render'></form> <div id='form_output'></div> </div>");
  ...
  DesignView.prototype.addQuestion = function(options) {
    ...
    $("#questions").append("<div data-repeat='false' class='question-definition' id='" + id + "'> <div class='aQuestion'> <label class='questionText' for='label-" + id + "'>Question -" + id + "</label> <input class='designFormTextInput' type='text' name='label-" + id + "' id='label-" + id + "' value='" + label + "'></input> <div class='questionInfo'> <div class='questionType'>Response Type: " + type + "</div> <label class='requiredOrNot' for='required-" + id + "'>Required</label> <input type='checkbox' class='requiredOrNot' name='required-" + id + "' id='required-" + id + "' " + (required === "false" ? "" : "checked='true'") + "></textarea> <div class='deleting'> <label class='deleteButton' >Delete</label> <button class='deleteButton' title='delete'><img src='images/delete.png'></button> </div> </div> </div> <div class='advancedOptions advanced-hidden'> <label class='advanced' for='validation-" + id + "'>Validation</label> <textarea class='advanced validation' name='validation-" + id + "' id='validation-" + id + "'>" + validation + "</textarea> <label class='advanced' for='action_on_questions_loaded-" + id + "'>Action on Questions Loaded</label> <textarea class='advanced' name='action_on_questions_loaded-" + id + "' id='action_on_questions_loaded-" + id + "'>" + action_on_questions_loaded + "</textarea> <label class='advanced' for='skip_logic-" + id + "'>Skip Logic</label> <textarea class='advanced' name='skip_logic-" + id + "' id='skip_logic-" + id + "'>" + skip_logic + "</textarea> <label class='advanced' for='action_on_change-" + id + "'>Action on Change</label> <textarea class='advanced' name='action_on_change-" + id + "' id='action_on_change-" + id + "'>" + action_on_change + "</textarea> </div> " + ((function() {
      switch (type) {
        case "select":
...
//# sourceMappingURL=DesignView.js.map
