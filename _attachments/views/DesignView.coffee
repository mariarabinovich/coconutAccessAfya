class DesignView extends Backbone.View
  ...
  render: =>
    ...
    @basicMode()
      ...
      # moved from html below: <div class='advanced'><b>Advanced: </b><p>Use <img title='repeat' class='designIcons' src='images/repeat.png' /> to make the question repeatable. If you want to group questions together to form a repeatable block then click <img title='group' class='designIcons' src='images/group.png'/> between the questions and use the <img title='repeat' class='designIcons' src='images/repeat.png' /> as before. Ungroup by using <img title='ungroup' class='designIcons' src='images/ungroup.png' />.</p>
      # </div>
  ...
  template: Handlebars.compile "
    <div id='design-view'>
      <h3>Design</h3>
      <small >
        <b class='instructions'>Instructions</b>: <p class='instructions'>Use the drop down below to select the type of questions that you will be asking. Click <button>Preview</button> to see what the questions will look like.</p>
      </small>
      <hr/>
    ...
    <div class='sortable' id='questions'>
        <label class='questionText' for='rootQuestionName'>Name</label>
        <input class='designFormTextInput' id='rootQuestionName' name='rootQuestionName' type='text'/>
      </div>


      <div class='designControls'>
      ...
      <button class='designActionsButton addButton'>Add</button><br/>
      <button class='designActionsButton' type='button'>Save</button>
      <button class='designActionsButton'>Preview</button>
      <button class='designActionsButton'>Advanced Mode</button>
      </div>
      <form id='render'></form>
  ...
  addQuestion: (options) ->
    ...
    #       <button class='deleteButton' title='repeat'><img src='images/repeat.png'></button>
    #       <input type='hidden' id=repeatable-#{id} value='false'></input>
    #       <button class='deleteButton' title='delete'><img src='images/delete.png'></button>
    #     </div>
    ...
    $("#questions").append "
      ...
      <div data-repeat='false' class='question-definition' id='#{id}'>
        ...
        <div class='aQuestion'>
          ...
          <label class='questionText' for='label-#{id}'>Question -#{id}</label>
          <input class='designFormTextInput' type='text' name='label-#{id}' id='label-#{id}' value='#{label}'></input>

          <div class='questionInfo'>


            <div class='questionType'>Response Type: #{type}</div>
            <label class='requiredOrNot' for='required-#{id}'>Required</label>
            <input type='checkbox' class='requiredOrNot' name='required-#{id}' id='required-#{id}' #{if required is "false" then "" else "checked='true'"}></textarea>
            <div class='deleting'>
              <label class='deleteButton' >Delete</label>
              <button class='deleteButton' title='delete'><img src='images/delete.png'></button>
            </div>
          </div>
        ...
        <div class='advancedOptions advanced-hidden'>
  ...
  advancedMode:->
    ...
    $('.advancedOptions').removeClass('advanced-hidden')
    $('.advancedOptions').addClass('advanced-visible')
    $('button:contains(Advanced Mode)').html "Basic Mode"

  basicMode:->
    # $('body').addClass("all-advanced-hidden")
    $('.advancedOptions').addClass('advanced-hidden')
    $('.advancedOptions').removeClass('advanced-visible')
